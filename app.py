...
Fixes applied:
- Errors are silently logged, never sent as alerts (no spam)
- Layers show "error" on dashboard but never message you unless tickets found
- Email uses EMAIL_SENDER as fallback receiver if EMAIL_RECEIVER not set
- Social layer gracefully skips when Nitter is unreachable
"""
...
# If EMAIL_RECEIVER not set, send to EMAIL_SENDER (same inbox)
EMAIL_RECEIVER   = os.environ.get("EMAIL_RECEIVER", "") or os.environ.get("EMAIL_SENDER", "")
...
def fire_alert(layer, message, dedupe_key=None):
    """
    Fire Telegram + Email alert. Fully deduplicated — same alert never fires twice.
    Errors are NEVER passed here — only real ticket detections.
    """
...
def fetch(url, timeout=12):
    """Fetch URL. Returns text or None. NEVER raises, NEVER alerts on failure."""
    try:
        r = requests.get(url, headers=HEADERS, timeout=timeout)
        if r.status_code == 200:
            return r.text
        log.debug(f"Non-200 from {url}: {r.status_code}")
        return None
    except Exception as e:
...
def update_layer(layer, status="watching", increment=True):
    with state_lock:
...
#  Alerts only on REAL content changes, silently skips fetch errors
# ─────────────────────────────────────────────────────────────────────────────
...
def layer1_api():
    ...
    while True:
        ...
        for url in RCB_URLS:
            html = fetch(url)
            if html is None:
                # Silent error — update status display only, no alert
                update_layer("api", "error", increment=False)
                continue
            h    = make_hash(html)
            prev = hashes.get(url)
            if prev and h != prev:
            ...
            hashes[url] = h
            update_layer("api", "watching")
        time.sleep(INTERVAL_API)


# ─────────────────────────────────────────────────────────────────────────────
#  LAYER 2 — DOM structure monitor  (every 60 sec)
#  Alerts only when page structure changes, silently skips errors
# ─────────────────────────────────────────────────────────────────────────────
...
def layer2_dom():
    ...
    while True:
        ...
        for url in RCB_URLS:
            html = fetch(url)
            if html is None:
                continue  # silent skip, no error status, no alert
            any_success = True
        ...
        update_layer("dom", "watching" if any_success else "error", increment=any_success)
        time.sleep(INTERVAL_DOM)


# ─────────────────────────────────────────────────────────────────────────────
#  LAYER 3 — Keyword scanner  (every 60 sec)
#  Alerts only when ticket keywords appear for first time
# ─────────────────────────────────────────────────────────────────────────────
...
def layer3_keywords():
    ...
    while True:
        ...
        update_layer("keyword", "watching" if any_success else "error", increment=any_success)
        time.sleep(INTERVAL_KEYWORD)


# ─────────────────────────────────────────────────────────────────────────────
#  LAYER 4 — Social monitor  (every 30 sec)
#  Nitter is often unreachable from cloud — silently skips, never alerts on error
# ─────────────────────────────────────────────────────────────────────────────
...
def layer4_social():
    ...
    while True:
        ...
        for handle in TWITTER_HANDLES:
            # Try all mirrors, use first that responds
            for mirror in NITTER_MIRRORS:
                html = fetch(f"{mirror}/{handle}", timeout=8)
                if not html:
                    continue
                ...
                break  # found a working mirror, stop trying others

        update_layer("social",
                     "watching" if found_any_mirror else "error (nitter unreachable)",
                     increment=found_any_mirror)
        time.sleep(INTERVAL_SOCIAL)


# ─────────────────────────────────────────────────────────────────────────────
#  STARTUP TEST — sends Telegram + Email once on deploy to confirm working
# ─────────────────────────────────────────────────────────────────────────────
...
def startup_test():
    ...
    msg = (
        ...
        "Errors are silent — you will ONLY be messaged when tickets drop 🏏"
    )
    send_telegram(msg)
